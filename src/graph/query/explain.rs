...
impl ExplainOutput {
    ...
    fn fmt_op(op: &PhysicalOp) -> String {
        match op {
            PhysicalOp::NodeScan { binding_col, label_filter } => format!(
                "NodeScan(col={binding_col}, label={label_filter:?})"
            ),
            PhysicalOp::VectorSeed { binding_col, query_param, k, contract } => format!(
                "VectorSeed(col={binding_col}, param={query_param}, k={k}, contract={contract:?})"
            ),
            PhysicalOp::Expand { src_col, dst_col, rel_type_filter, direction, .. } => format!(
